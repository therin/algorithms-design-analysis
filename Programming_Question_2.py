...
Your task is to compute the total number of comparisons used to sort the given input file by QuickSort. As you know, the
 number of comparisons depends on which elements are chosen as pivots, so we'll ask you to explore three different pivoting rules.
You should not count comparisons one-by-one. Rather, when there is a recursive call on a subarray of length m, you should
 ...
 −1 elements in the subarray in this recursive call.)
WARNING: The Partition subroutine can be implemented in several different ways, and different implementations can give 
you differing numbers of comparisons. For this problem, you should implement the Partition subroutine exactly as it 
...
def partition(left, right):
    integer_array[left], integer_array[right - 1] = integer_array[right - 1], integer_array[left]
...
def partition(left, right):
    mid = (left + right - 1) / 2
    if (integer_array[left] <= integer_array[mid] and integer_array[mid] <= integer_array[right - 1]) or (integer_array[left] >= integer_array[mid] and integer_array[mid] >= integer_array[right - 1]):
        integer_array[left], integer_array[mid] = integer_array[mid], integer_array[left]
    elif (integer_array[left] <= integer_array[right - 1] and integer_array[right - 1] <= integer_array[mid]) or (integer_array[left] >= integer_array[right - 1] and integer_array[right - 1] >= integer_array[mid]):
        integer_array[left], integer_array[right - 1] = integer_array[right - 1], integer_array[left]
    pivot = integer_array[left]
    i = left
    for j in range(left + 1, right):
